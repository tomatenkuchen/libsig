...
TEST_CASE("power analysis tests", "[P]") {
  ...
  REQUIRE_THAT(res_power.number(),
               Catch::Matchers::WithinRel(W<double>(230).number(), 0.01));
  REQUIRE_THAT(res_voltage.number(),
               Catch::Matchers::WithinRel(V<double>(230).number(), 0.01));
  REQUIRE_THAT(res_current.number(),
               Catch::Matchers::WithinRel(A<double>(1.0).number(), 0.01));

  // long sinus that has a phase delay of 90 degrees, and therefore should
  // result to 0 active power
  std::vector<V<double>> const v_sinus_long =
      create_signal<V<double>>(SignalType::Triangular, 2000, ms<double>(100),
                               Hz<double>(50), V<double>(325));
  ...
  REQUIRE_THAT(res_power_phased.number(),
               Catch::Matchers::WithinAbs(W<double>(0).number(), 0.01));

  // triangle signal
  std::vector<V<double>> const v_triangle =
      create_signal<V<double>>(SignalType::Triangular, 2000, ms<double>(100),
                               Hz<double>(50), V<double>(325));
  ...
  BENCHMARK("voltage computation of 10000 samples") {
    return sig::rms_voltage(v_signal);
  };
  BENCHMARK("current computation of 10000 samples") {
    return sig::rms_current(i_signal);
  };
}
