...
std::vector<T> make_random_vector(int size, std::uniform_int_distribution<T> distribution) {
...
TEST_CASE("filter test integer", "[filter]") {
  ...
  SECTION("feed data to zero initialized filter") {
    ...
    std::ranges::for_each(random_vector, [&](int random_number) { ma.update(random_number); });

    REQUIRE_THAT(ma.output(), Catch::Matchers::WithinRel((d.a() + d.b()) / 2, 0.1));
  }
...
TEST_CASE("benchmarking filter", "[filter][benchmark]") {
  ...
  BENCHMARK("integral filter with power of 2 strength") { return ma_i2.update(2); };
  BENCHMARK("integral filter with power of 2 stregth and non power of 2 input") {
    return ma_i2.update(7);
  };
  BENCHMARK("integral filter with non-power of 2 strength and power of 2 input") {
    return ma_i7.update(8);
  };
  BENCHMARK("integral filter with non-power of 2 strength and non power of 2 input") {
    return ma_i7.update(13);
  };
  BENCHMARK("float filter with power of 2 strength") { return ma_f2.update(2.0); };
  BENCHMARK("float filter with power of 2 strength and non power of 2 input") {
    return ma_f2.update(7.0);
  };
  BENCHMARK("double filter with power of 2 strength") { return ma_d2.update(2.0); };
